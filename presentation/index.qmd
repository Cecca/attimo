...
Consider all pairs of subsequences of length $w$ of a given time series.
...
The key point is that we only compute the distance of
subsequences falling into the same bucket.
:::
...
```{python}
#| layout-ncol: 2
for seed in [1234, 243,256,23562,235,2]:
...
:::{.absolute bottom=0 left="54%" fragment-index=2}
```{python}
plot_success_p(w, 1, k=2, ls=[1, 10, 100])
...
- $L$ maximum number of repetitions,
- $K$ maximum number of concatenations.
...
k=4
dist = 1
plot_execution(w, r=1, k=k, max_reps=100, rep=1, p_threshold=0.9, dist=dist + 1)
...
:::{.fragment .bg-white}
```{python}
plot_execution(w, r=1, k=k, max_reps=100, rep=10, p_threshold=0.9, dist=dist)
```
...
:::{.fragment .bg-white}
```{python}
plot_execution(w, r=1, k=k, max_reps=100, rep=100, p_threshold=0.9, dist=dist)
```
...
```{python}
k=3
plot_execution(w, r=1, k=k, max_reps=100, rep=1, p_threshold=0.9, dist=dist)
```
...
:::{.fragment .bg-white}
```{python}
plot_execution(w, r=1, k=k, max_reps=100, rep=15, p_threshold=0.9, dist=dist)
plt.savefig("imgs/execution5.png", dpi=300)
```
...
- Compute dot producs for hash values in the frequency domain (also done in the first versions of the Matrix Profile)
...
- For time series of a few millions and above, with a not-to-small relative
  contrast, try `Attimo`
