...
fn main() -> Result<()> {
    ...
    let stopping_condition = |d: f64, prefix: usize, repetition: usize| {
        ...
        repetition as f64 >= (num_motifs as f64 / delta).log(std::f64::consts::E) / p.powi(prefix as i32)
    };
    ...
    loop {
        ...
        let mut new_topk = (rep..coll.current_repetitions()).into_par_iter().map(|rep| {
            let mut topk = TopK::new(num_motifs, exclusion_zone);
        ...
        let (candidate_prefix, candidate_reps) = coll.best_move(topk.first_not_confirmed().unwrap().distance, delta / num_motifs as f64);
